...
export default async function ShisekiDetailPage({
  ...
  return (
    <div className="max-w-[760px] mx-auto px-6">
      <div className="pt-12">
        <Link href="/shiseki" className="text-textMuted text-sm hover:text-textMain transition-colors inline-block">
          ← {LABELS.shiseki.text}
        </Link>
      </div>
      ...
      <div className="mt-8">
        <div className="flex items-baseline gap-3 mb-3">
          <span className="mono font-bold text-[15px] text-textMuted">
        ...
        <dl className="flex flex-wrap gap-x-8 gap-y-2 mt-5 text-[13px]">
          {item.location && (
            <div className="flex gap-2">
              <dt className="text-textMuted whitespace-nowrap">ところ</dt>
              <dd className="text-textMain">{item.location}</dd>
            </div>
          )}
          {item.era && (
            <div className="flex gap-2">
              <dt className="text-textMuted whitespace-nowrap">年代</dt>
      ...
      {item.confidence === "check" && (
        <p className="text-[12.5px] text-textMuted mt-4 leading-relaxed">
      ...
      {item.sessions && item.sessions.length > 0 && (
        <section className="mt-12 border-t-[1.5px] border-textMain">
          <h2 className="text-[12.5px] font-bold tracking-[0.1em] text-textMuted pt-4 pb-1.5">
          ...
          {item.sessions.map(s => (
            ...
            >
              <span className="text-[14px] font-bold text-textMain min-w-0">
                {s.title}
                <span className="block text-[11.5px] text-textMuted font-normal mt-1">
                  「{s.entity}」に触れています
                </span>
              </span>
              <span className="mono text-[11.5px] text-textMuted whitespace-nowrap justify-self-end">
                {formatDate(s.date)}
