  ...
  return (
    ...
    >
      ...
      <span className="text-[15px] font-bold">
        {label}
        <span className="block md:inline text-textMuted font-normal text-[12.5px] md:ml-3.5">{desc}</span>
      </span>
      <span className="mono text-textMuted transition-colors group-hover:text-accent">→</span>
    </Link>
  )
}
    ...
    <div className="max-w-[1040px] mx-auto px-6">
      ...
      {latest && (
        <div className="pt-11">
          <PlateFrame className="pt-10 px-6 pb-9 md:px-11">
            ...
            <p className="mono text-[13px] text-textMuted mb-2.5">{dateRangeDisplay(latest)}</p>
            ...
            <p className="text-[13px] text-textMuted mb-5">{latest.officialTitle}</p>

            <div className="flex flex-wrap gap-2 mb-7">
              {latest.tags.map(tag => (
                <span
                  key={tag}
                  className="text-[11.5px] font-medium text-textMuted border border-lineStrong rounded-[3px] px-[11px] py-[3px]"
                >
                  {tag}
                </span>
              ))}
          ...
          {/* 機能文 */}
          <div className="flex flex-wrap justify-between gap-3 pt-[26px] text-[13.5px] text-textMuted">
      ...
      <section className="mt-[52px] border-t-[1.5px] border-textMain">
        <div className="flex items-baseline justify-between pt-4 pb-1.5">
          <h2 className="text-[12.5px] font-bold tracking-[0.1em] text-textMuted">近時の記録</h2>
          <Link href="/gikai/sessions" className="text-[12.5px] text-textMuted transition-colors hover:text-accent">
            すべての記録 →
          </Link>
        </div>
        {sessions.slice(0, 4).map((s, i) => (
          ...
          >
            ...
            <span className="mono text-textMuted text-[13px] hidden md:block">{toDot(s.date)}</span>
            <span className="text-[14px] font-bold leading-snug">
              {s.narrativeTitle ?? s.officialTitle}
              <span className="mono block md:hidden text-textMuted text-[11px] font-normal mt-0.5">{toDot(s.date)}</span>
            </span>
            <span className="text-[11px] text-textMuted border border-lineStrong rounded-[3px] px-2 py-[2px] whitespace-nowrap justify-self-end">
              {meetingTag(s.tags)}
            </span>
          </Link>
        ))}
