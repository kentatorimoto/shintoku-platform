    ...
    <div className="max-w-[1040px] mx-auto px-6">
      ...
      <div className="mt-11">
        <PlateFrame className="px-6 py-8 md:px-10">
          <div className="flex justify-between text-[11.5px] text-textMuted mb-2">
          ...
          <p className="text-[12.5px] text-textMuted mb-7">{featured.summary}</p>
                ...
                <div key={s.sessionId} className={`relative pl-[22px] ${last ? "" : "pb-6"}`}>
                  <span
                    className={`absolute left-[-18px] top-[9px] w-[11px] h-[11px] rounded-full border-[1.5px] ${
                      now ? "bg-accent border-accent" : "bg-paper border-textSub"
                    }`}
                    aria-hidden
                  />
                  <div className="mono text-[12px] text-textMuted">{toDot(s.date)}</div>
                  <div className={`text-[14.5px] font-bold mt-0.5 ${now ? "text-accent" : "text-textMain"}`}>{s.title}</div>
                  <div className="text-[13px] text-textMuted mt-0.5 max-w-[640px]">{s.conflict}</div>
      ...
      <section className="mt-14 border-t-[1.5px] border-textMain">
        <div className="flex items-baseline justify-between pt-4 pb-1.5">
          ...
          <Link href="/process/issues" className="text-[12.5px] text-textMuted hover:text-accent transition-colors">論点カード →</Link>
        </div>
          ...
          return (
            ...
            >
              <span className="text-[15px] font-bold min-w-0">
                {issue.title}
                <span className="block text-[12px] text-textMuted font-normal mt-0.5">{issue.lede}</span>
              </span>
              ...
              <span className="mono text-[11.5px] text-textMuted text-right whitespace-nowrap justify-self-end">
      ...
      {/* ── 関連ページ（サブページへの導線を残す）─────────────────────── */}
      <section className="mt-14 border-t-[1.5px] border-textMain">
        <h2 className="text-[12.5px] font-bold tracking-[0.1em] text-textMuted pt-4 pb-1.5">関連</h2>
        {[
          ...
          >
            <span className="text-[15px] font-bold">
              {l.label}
              <span className="text-textMuted font-normal text-[12.5px] ml-3">{l.desc}</span>
            </span>
