...
const TYPE_COLORS: Record<string, string> = {
  ...
  質問: "text-textMuted border-line",
  報告: "text-textMuted border-line",
  設計: "text-textMuted border-lineStrong",
...
function Source({ event }: { event: TuktukEvent }) {
  ...
  if (event.sourceNote) {
    return (
      <span className="text-xs text-textMuted">{event.sourceNote}</span>
    )
  }
  return null
}
  ...
  return (
    <div className="max-w-[1040px] mx-auto px-6">
      ...
      <div className="pt-12 pb-2 mb-8">
        ...
        <p className="text-textSub text-base leading-relaxed">
      ...
      <div className="space-y-8">
        {PHASES.map((phase) => (
          ...
          >
            <h2 className="text-sm font-semibold text-textMuted tracking-wide mb-5">
              {phase.label}
            </h2>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      <span
                        className={`text-xs font-semibold px-1.5 py-0 rounded border ${
                          TYPE_COLORS[ev.type] ?? "text-textMuted border-line"
                        }`}
                      >
                        {ev.type}
                      </span>
      ...
      <section className="mt-10 bg-ink border border-line rounded-[3px] p-6">
        <h2 className="text-sm font-semibold text-textMuted tracking-wide mb-3">
          名称ゆれ（観測用メモ）
        </h2>
        <ul className="space-y-1">
          {NAME_VARIANTS.map((v) => (
            <li key={v} className="text-sm text-textSub">
              {v}
            </li>
          ))}
        </ul>
      ...
      <section className="mt-4 bg-ink border border-line rounded-[3px] p-6">
        ...
        <dl className="grid grid-cols-[auto_1fr] gap-x-6 gap-y-1 text-sm">
          <dt className="text-textMuted">開所日</dt>
          <dd className="text-textSub">{FACILITY.openDate}</dd>
          <dt className="text-textMuted">延床面積</dt>
          <dd className="text-textSub">{FACILITY.floorArea}</dd>
          <dt className="text-textMuted">事業費</dt>
          <dd className="text-textSub">{FACILITY.projectCost}</dd>
        </dl>
      </section>
      ...
      <section className="mt-4 bg-ink border border-line/40 rounded-[3px] p-6">
        ...
        <ul className="space-y-1.5">
          {UNRESOLVED.map((item) => (
            <li key={item} className="text-xs text-textMuted leading-relaxed">
              ・{item}
            </li>
          ))}
        </ul>
