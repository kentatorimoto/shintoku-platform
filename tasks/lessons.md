...
## 2026-09-24 要点カードは会期の締めくくりを落とす

複数パートのMDをまとめて渡しても、8枚の枠は初日の大型議案（料金改定・条例制定）で埋まり、
**最終日に採決された決算認定3件が1枚も残らなかった**。生成器は「大きい出来事」を選ぶが、
会期の締めくくりが重要だとは知らない。→ `scripts/prompts/cards.md` に
「最終日に採決された主要議案の結果は必ず1枚含める」を明示した。

## 2026-09-24 数値は期間と対象範囲ごと運ぶ

MDの「令和7〜12年度にかけて約28億円の事業費で**各施設**を更新」を、カードが
「上美育成牧場を約28億円かけて更新」と1施設・期間なしに縮めた。単年度の解体費は775万円で、
桁が3つ違う。**範囲を縮めた要約は、数値を書き換えていないのに事実を変える。**
