# 読者ファースト化（並び替え／翻訳層／要点カード）

指示書: 「一般の町民が面白く読める → そこから議会の意思決定に興味を持つ」順に設計し直す。
データ層（`content/`・schema・パイプライン）の正式語彙は変えない。変えるのは **UIの語彙と順序**、
および **要点カードという新しい表示層**。

PR構成: Phase A+B = `feat/reader-first`（UIのみ）／ Phase C = `feat/session-cards`（別PR）。

---

## Phase A: セッションページの並び替え（逆ピラミッド）

- [x] `SessionDetail.tsx` の並びを summary →（カード/スライド）→ 動画 → 議案審議/一般質問 に変更
- [x] 各セクションの内部構造は変更なし（並び替えのみ）
- [x] 議案審議に `#giketsu`・一般質問に `#qna` アンカー（`scroll-mt-20` でヘッダー分を逃がす）
- [x] summary カード直下にページ内リンク（「この会議で決まったこと 36件 ↓」）を追加
- [x] スライドなしパートはセクション非表示（PR #6 の実装のまま）

## Phase B: 翻訳層（UIラベルの生活語化）

- [x] `lib/labels.ts` 新設。UI表示ラベルを一元管理（`{ text, formal }`）
- [x] ヘッダーサブ → 「町のことが、どう決まっているか。」／正式名称はフッターのコピーライト行に残す
- [x] 議案審議 → 「この会議で決まったこと」（正式併記あり）
- [x] 一般質問 → 「議員が聞いたこと、町の答え」（正式併記あり）
- [x] 委員会付託 → 「委員会でくわしく審査することになったもの」（正式併記あり）
- [x] 継続論点 → 「つづいている話」（トップの索引・`/process` の索引見出し）
- [x] 動画アーカイブ → 「会議の動画」（正式併記なし）
- [x] 行政報告 → 「町からの報告」を `labels.ts` に定義（UI未実装のため定義のみ）
- [x] narrativeTitle・タグ・採決結果チップは変更しない

### 保留（Kenta の判断待ち）

- **SHEET番号・座標などの英字ラベルは今回維持**（デザイン署名のため）。試用判定の際に翻訳層と合わせて再評価する
- 予算・決算特別委員会パートの見出し「予算審査 — 項目ごとの質疑」は現行文言のまま `labels.ts` へ移送。
  生活語化するかは未定（`LABELS.qnaCommittee`）
- QnaItem 詳細の `dt` ラベル（質問／行政の回答／結論／継続課題）は現行維持。
  「継続課題」は `/process` の「継続論点」とは別語なので、指示書の対応表の対象外と判断した
- `GlobalSearch` のカテゴリチップ（セッション／一般質問／議決）は短いラベルなので正式語のまま

## Phase C: 要点カード（スライドの後継）＋ OGP

- [x] `scripts/generate-cards.ts`（`npm run cards:generate -- <sessionId>`）
      - 全パートMDが `reviewed: true` でなければ exit 1（実際に day2 を false にして確認）
      - `scripts/prompts/cards.md` にプロンプト（5〜8枚・MDの事実のみ・評価語禁止・中学生の語彙）
      - 生成後 `build:data` のバリデータで検証し、落ちたらエラーを返して自己修正（最大2回）
- [x] `docs/content-schema.md` §11 として cards.yaml のスキーマを追加（v1.3）
- [x] `build-data.ts` に cards.yaml のバリデーション＋ `public/data/cards/{id}.json` 変換
- [x] `components/SessionCards.tsx`（summary 直下・横スワイプ・`1 / 7` 表示・スナップスクロール）
- [x] 旧スライドは、カードがあるセッションでは「過去のスライド」として折りたたみ
- [x] `app/gikai/sessions/[id]/opengraph-image.tsx`（next/og）＋ `app/opengraph-image.tsx`（汎用）
- [x] `tasks/add-session.md` に「reviewed:true → cards:generate → 確認 → reviewed:true」を追記
- [x] `CLAUDE.md` を更新（cards.yaml・UI表示順・labels.ts・人間に残る判断4つ）
- [x] `r8-2026-06-regular-2` でカード生成し、cards.yaml をPRに含める（トーンの基準作り）
- [x] 過去セッションへのカード遡及生成はスコープ外

---

## レビュー

### Phase A+B（`feat/reader-first`）

- `npm run build` 成功（80ページSSG、`public/data/` の生成物に差分なし＝データ層は無変更）
- `npx tsc --noEmit` クリーン。`npm run lint` の既存6エラー（`scripts/test-scraper.ts` 等）以外に新規指摘なし
- 変更ファイル: `lib/labels.ts`(new), `SessionDetail.tsx`, `[partIndex]/page.tsx`,
  `components/Header.tsx`, `components/Footer.tsx`, `app/page.tsx`, `app/process/page.tsx`

#### 設計判断

- **セクション見出しは `{ text, formal }` の2行組**（生活語15px太字＋正式名称11.5px）。
  生活語だけに置き換えると正確性が落ちるため、対応が常に画面上で辿れる形にした
- **`qnaLabel(speakerRole)` を `labels.ts` に置いた。** 見出し（SessionDetail）と導線（page.tsx）で
  同じ分岐を二重に書くと、予算委員会パートでラベルがずれるため
- **導線は summary カードの内側に入れた。** 独立カードにすると読者の視線が summary から一度切れる

### Phase C（`feat/session-cards`）

- `r8-2026-06-regular-2` のカード7枚を自己修正0回で生成（入力 9,625 / 出力 2,920 トークン、約 $0.073）
- `npm run build` 成功（99ページ = ページ80 + OGP19）。`npx tsc --noEmit` クリーン、新規ファイルに lint 指摘なし
- 数値はMDと照合済み（3億6498万7000円・90億618万7000円・1億8733万円・263台・1813万9100円・工期/納期）

#### 設計判断

- **カードの品質ゲートはスクリプト側に置いた。** 全パートMDが `reviewed: true` でなければ生成しない。
  「カードはレビュー済みコンテンツの派生物」という前提を、運用の約束ではなく機械で守る
- **ヘッダ（generated_by / generated_at / reviewed）はスクリプトが決定的に生成し、AIには `cards:` だけ書かせる。**
  extract-md.ts と同じ規律。監査用フィールドをAIに触らせない
- **`callClaude` / `logUsage` / `stripFences` は extract-md.ts から export して共有した。**
  モデル呼び出しの規律（ストリーミング・プロンプトキャッシュ・max_tokens 検査）を1箇所に保つため
- **OGPの和文フォントは `npm run og:fonts` でビルド前に1回だけ取得する。**
  `next build` は7ワーカー並列で画像を作るため、各ワーカーが 5.4MB のフォントを同時に取ると
  取得が壊れて fontkit が "Bad flags" で落ちた（5セッションで再現）。キャッシュを `.cache/og-fonts/` に置いて解決
- **和文フォントのサブセットは同梱しない。** 新しいセッションに未知の漢字が出ても欠けないようにするため
- **Space Mono（98KB）だけは `assets/fonts/` に同梱する。** フォントを1つも渡さないと satori は
  "No fonts are loaded" で落ち、**ビルド全体が失敗する**（URLを壊して実測）。1つでも渡っていれば
  next/og の既定フォントが和文を拾うので、Zen Old Mincho が取れないときも
  「見出しが明朝でなくゴシックになる」だけで済む。OFL-1.1（`assets/fonts/OFL.txt`）

#### レビュー反映（Kenta 判断、2026-08-16）

- **`kind: report` を新設**（行政報告に由来する事実）。`decision` は議案の採決結果に限る。
  帯広信用金庫の増設保留は `report` へ。UI上の種別表示は「報告」
- 農業委員のカードは「11名が決まった／再任を含む11名に議会が同意した」に修正。
  「新体制」は程度・新規性の修飾語で、MD本文にその語がない
- headline の detail は「6/3に延期が報告され、6/19の質疑で資材・労務単価の上昇が理由と説明された」に。
  どのパートの出来事かが読者に伝わる書き方にした（link は day1 のまま）
- `scripts/prompts/cards.md` に規律を2行追加（`decision` の範囲／程度・新規性の修飾語はMDにある語だけ）
- `cards.yaml` を `reviewed: true` に

---

# r8-2026-09-regular-3（PR #12）のレビュー

## 判明した課題（このPRのスコープ外）

- **「議会基本条例」に対応するテーマタグが既知一覧にない。** `docs/content-schema.md` §10 の
  既知テーマは インフラ / 農業 / 観光 / 宿泊税 / 教育 / 文化 / 子育て / 財政 / 医療 / 物価高騰対策 /
  総合計画 / エネルギー / 人口政策 の13個で、議会運営・自治制度を指す語がない。
  今回は新設せず `[定例会, 決算, 財政, インフラ]` で運用した。
  「議会運営」あるいは「自治制度」を既知テーマに足すかは、同種の議案がもう1件出てから判断する
- 議案第69号・第70号は補正予算だが、議案種別タグは0〜1個の制約があるため `決算` を採った
  （決算3件の特別委員会付託がこの会期の主題のため）

## 追って照合するもの

- **水道料金の数値は、広報の料金表が公開されたら照合する。** 動画確認で
  「口径13mm 基本料金 現行1,201円 → 改正後1,200円」「超過料金 134円 → 135円」
  「営農用 1m³あたり120円」を採ったが、平均改定率6.19%増との関係が数字だけでは追えない
  （基本料金がほぼ据え置きで、逓増料金化と用途別料金の廃止で上がる設計と思われる）。
  施行は令和9年4月1日

---

# 郷土資料の統合（PR `feat/archive-shiseki`）

## 積み残し: 歴史的呼称と現代呼称の対応表を別レイヤーで持つ

史跡と議会記録の接続（スキーマ §12.7）は **`entities` の完全一致のみ**で、現在つながるのは
26件中2件（`s10` トムラウシ／`s23` 新得山）。仕様書が例示した
「旧狩勝トンネル → 令和8年9月定例の狩勝ポッポの道の橋梁補修」は成立しない。

原因は語彙のずれで、データの不備ではない:

| 原本（1994年） | 議会記録（2026年） |
|---|---|
| 狩勝峠 / 旧狩勝線 / 新内トンネル | 狩勝高原整備費 / 狩勝ポッポの道 |
| 旧狩勝牧場 | （給水設備の光熱費として言及） |
| 上川灌漑溝十勝川水門 | （現存せず。昭和53年に通水終了） |

**`entities` に「狩勝ポッポの道」を足す案は採らない**（2026-09-09 判断）。
原本にない語を正典に書き足すことになり、「原本は原本として保全する」という
ryuiki-archive の原則と、ATLAS の「事実の再配置のみ」の規律に反するため。

### 代わりに検討する案

**歴史的呼称 ↔ 現代呼称の対応表を、資料からも議会記録からも独立した第3のレイヤーとして持つ。**

```yaml
# 案: content/archive/place-aliases.yaml（正典ではなく索引）
- canonical: 狩勝
  historical: [狩勝峠, 旧狩勝線, 狩勝トンネル, 狩勝国境]
  current: [狩勝高原, 狩勝ポッポの道, 狩勝峠園地]
  note: 旧狩勝線の廃線敷が遊歩道「狩勝ポッポの道」として整備された
  confidence: check          # 対応関係の確度も持つ
```

- どちらのレイヤーにも語を足さず、**対応関係だけを別に記録する**。原本も議会記録もそのまま残る
- 接続時は「原本の語 → canonical → 現代の語」で引く。UIでは根拠として
  「旧狩勝線（現在の狩勝ポッポの道）」のように**変換の経路を見せる**
- 対応表自体に `confidence` を持たせ、推定を含むものは `check` として区別する
  （史跡の `confidence: check` と同じ思想）

判断が要る点:
- 誰がこの対応表を書くか（原本にも議会記録にも書かれていない知識であり、**編集者の判断が入る**）
- 入れる範囲。地名だけか、施設・制度（駅逓所→道の駅、灌漑溝→用水路）まで広げるか
- 「町に聞く」（RAG）でも同じ対応表が要るはずなので、その設計と一緒に決めるのが自然

## そのほかの積み残し

- 郷土読本くったりの**下巻が未スキャン**。上巻10章はメタのみ整備済みで、UIには出していない
- 全町略図の凡例 `map_no: 2`「伊藤傳五郎住居跡」（p.40-41）は本文に独立見出しがなく `s24` は欠番。
  原本照合で確定したら追加する
- `confidence: check` の2件（`s12` ペンケ沢駅逓所跡／`s13` 広内尋常小学校跡地）は、
  学校群の境界が原本未確認。UIには「原本と照合中」と出している
- 権利の確認（発行元＝新得町郷土研究会）は継続検討中。現在は許諾なしで守れる範囲＝概要レベルに限定

---

# 小豆（暗色）反転の廃止（2026-09）

## 決定

**`/process` 配下だけを小豆（暗色）に反転する仕組みを廃止し、全ページを緑灰（亜麻）に統一した。**

デザイン刷新（PR #7）では「意思決定の流れ＝別の紙」という意味づけで `/process` だけ紙を変えていた。
**セクションが増えて、この例外が成立しなくなった**ため廃止する。

- `/shiseki`（土地の記憶）が加わり、「議会記録」「意思決定の流れ」「土地の記憶」の3系統になった。
  このうち1つだけ紙が違う理由を説明できない
- ヘッダー・フッターは全ページ共通なので、`/process` に入るたびにナビ全体が反転していた
- 紙が変わることで、同じコンポーネント（PlateFrame・タグ・チップ）が2通りの見え方を持ち、
  デザイン確認のコストが常に2倍かかっていた

削除したもの: `app/process/layout.tsx` / `components/AzukiPaper.tsx` /
`app/layout.tsx` の FOUC 防止インラインスクリプト / `globals.css` の `[data-paper="azuki"]` ブロック。

## 解決済み: 透過付きテキストのコントラスト（PR: fix/contrast-scale）

透過による階調を全廃し、階調を3段のトークンとして `globals.css` に一元化した。
`npm run check:contrast`（`build` から呼ばれる）が回帰を止める。
使い分けは `docs/design/text-scale.md`。

### 実測して分かったこと

**1. 「textSub より薄い4段目」は作れない。** 紙の上で AA を満たす輝度の上限は
`L=0.138599`、当時の `textSub`（`#67685a`）が `L=0.135224`。余地は RGB 1段
（`#68695b` = 4.51:1）しかなく、そこに置いた色は `textSub` と見分けがつかない。

**2. `textSub` はカード地で元から未達だった。** 下表は紙（base）だけで測っていたが、
`.card` は `bg-ink`（`#dee0d1`）。その上では `#67685a` は **4.24:1** で未達。
透過をやめて実色に戻すだけでは AA に届かない。
そこで**紙とカード地の両方で AA を満たす3段**に組み直した。

| トークン | 旧 | 新 | 紙 | カード地 |
|---|---|---|---|---|
| `textMain` | `#23241c` | 据え置き | 12.66:1 | 11.70:1 |
| `textSub` | `#67685a` | `#4a4b40` | 7.16:1 | 6.62:1 |
| `textMuted` | （無し） | `#5f6052` | 5.17:1 | 4.78:1 |

**3. 茜も文字としては未達だった。** `#b8432c` は紙 4.38 / カード 4.05、
`btnPrimary` のラベル（onAccent on accent）も 4.38。薄める方向には振れないので
**一段濃く**して `#ae3b25`（紙 4.92 / カード 4.55 / ラベル 4.92）にした。

**4. 薄い茜地の上の茜文字は、どうやっても届かない。** `bg-accent/20` の上では
`accent` が 3.44、`accentSoft` に落としても 4.00。`.chipActive` が既に持っていた
「地は中立の `bg-hover`、現在地は茜の文字と罫線」の形に4箇所を寄せた。

**5. `--color-base` が Tailwind の `text-base` を潰していた。**
色トークン `base` があると `.text-base { color: … }` が生成され、文字サイズ 1rem の
`text-base` が消える。ビルド後のCSSで確認した（`.text-base{color:var(--color-base)}`）。
`paper` に改名して衝突を解いた。

### 旧記録（未達だった透過指定）

（以下は当時の記録。紙の上だけで測っていた）

統一後にコントラストを実測したところ、**`/process` 配下の12箇所が AA 未達**だった。
ただしこれは**小豆廃止で生じた問題ではなく、両方の紙で元から未達**だったもの（下表）。
緑灰の `textSub` は 4.58:1 と余裕がないため、透過をかけると割り込む。

| 指定 | 小豆（廃止前） | 緑灰（統一後） | 判定 |
|---|---|---|---|
| `textSub/80` | 4.05:1 | 3.16:1 | どちらも未達 |
| `textSub/70` | 3.46:1 | 2.69:1 | どちらも未達 |
| `textSub/60` | 2.91:1 | 2.28:1 | どちらも未達 |
| `textSub/50` | 2.43:1 | 1.95:1 | どちらも未達 |
| `textSub/40` | 2.01:1 | 1.68:1 | どちらも未達 |
| `accent/70` | 4.19:1 | 2.80:1 | どちらも未達 |

該当は `timeline/TimelineClient.tsx`（3）/ `priorities/page.tsx`（5）/ `issues/tuktuk/page.tsx`（3）
ほか。いずれも `text-xs` や `text-[11px]` の小さな補助テキストで、
「大きい文字は 3:1 でよい」という例外も使えない。

### 対処案（未着手）

1. **透過を使うのをやめる。** 補助テキストは `text-textSub` の実色で出す（4.58:1 で AA 達成）
2. それでも階層を作りたいなら、**さらに薄い階調のトークンを1つ足す**のではなく、
   サイズと余白で差をつける（`text-[11px]` ＋ 余白）
3. `text-accent/70` は、茜を薄めると 2.80:1 まで落ちる。**茜は薄めない**（実色 4.38:1 でも
   本文には足りないので、茜は見出し・リンク・現在地など「大きい文字か非テキスト」に限る）

透過の使用は `/process` 以外（`/gikai`・`SessionDetail` 等）にも広がっているので、
**翻訳層（`lib/labels.ts`）と同じように、階調も一箇所で決める**方が筋がよい。
別PRで、パレットの階調設計として扱う。

## 積み残し: /newsletters（広報誌検索）

**3.29 MB の HTML が出ていて、しかもどこからもリンクされていない。**

- `app/newsletters/page.tsx` が `public/data/newsletters_index.json`（3.26MB）を
  `fs` で読み、**まるごと props でクライアントコンポーネントに渡している**。
  そのため RSC ペイロードとして 3.26MB が HTML に直列化される
- 内部リンクは0件（Header・Footer・BottomNav・本文いずれからも辿れない）
- デザインも旧世代のまま（`.pageWrap` / `.pageTitle` / `.pageDesc`）

直し方は `/gikai/sessions`（PR #22）と同じ形にできるが、**その前に位置づけを決める。**
全文検索はすでに `GlobalSearch`（ヘッダーの虫眼鏡）が町報・議決・史跡・一般質問を
横断で持っている。`/newsletters` を残すのか、`GlobalSearch` に吸収するのか、
ナビに載せるのか——を決めてから対処する。

（2026-09-16 の性能調査で発見。PR #22 / #23 では触っていない）
