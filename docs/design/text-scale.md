...
静的検査は className を行単位で読むだけなので、次は見つけられない。

- **親子をまたぐ配色** — 地色が祖先にあり、文字が子孫にある場合
- **継承した文字色** — `text-` を1つも書いていない要素（装飾の丸や帯を除外するため、
  `text-` を含まない行は検査の対象外にしてある）
- **状態でしか出ない配色** — ホバー・選択中・展開後だけに出る組み合わせ
