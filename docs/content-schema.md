...
- 自然人の個人情報: 生年月日・生年（「昭和○年生まれ」等を含む）は記載しない。年齢のみ可。住所は地区名まで（丁目・番地は書かない）。法人・事業者の所在地はそのまま記載してよい。
...
| `items[]` | `id, title, order, page_start, page_end, confidence, reviewed, location, era, entities, summary` | `page_end`, `location`, `era`, `summary` |
